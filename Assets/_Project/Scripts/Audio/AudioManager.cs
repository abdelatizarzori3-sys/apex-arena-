...
namespace ApexArena.Audio
{
    public class AudioManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            PlayMusic(MusicState.Explore, true);
        }
        public void PlayMusic(MusicState state) { PlayMusic(state, false); }
        private void PlayMusic(MusicState state, bool force)
        {
            if (!force && currentMusicState == state && musicSource != null && musicSource.isPlaying) return;
        ...
        public void PlayVictory() { if (victoryStinger != null && musicSource != null) musicSource.PlayOneShot(victoryStinger); }
        public void PlayDefeat() { if (defeatStinger != null && musicSource != null) musicSource.PlayOneShot(defeatStinger); }
        ...
        private AudioClip FindSFX(string name) => weaponSounds.Find(c => c != null && c.name == name) ?? techSounds.Find(c => c != null && c.name == name);
        private AudioClip FindUISound(string name) => uiSounds.Find(c => c != null && c.name == name);
    }
    public enum MusicState { Explore, Tension, Combat, Victory, Defeat }
}
